...
import { get_data_table_items } from "helpers/data_tables"
...
const ListScreen = () => {
  const { loading_app, data, search_osrm_action } = useSearchOSRM({ entity: "items" })
  ...
  const data_table_items = get_data_table_items(current_data)

  return (
    <div className="w-full h-full flex flex-col items-center justify-start">
      <Breadcrumb data={[{ name: "Listado de artículos", href: "/items" }]} />
      <div className="w-full flex flex-col justify-center items-center py-4 px-6">
        <TitleScreen title="listado de artículos" />
        {/* <p>ItemListScreen</p> */}
      </div>
      <div className="w-full flex flex-col gap-2">
        <div className="w-full flex flex-col lg:flex-row justify-between items-end lg:items-center gap-4 px-6">
          <SearchBarFilter />
          <ButtonListNavigation label="Crear artículo" route="/items/new" />
        </div>
        <div className="w-full flex flex-col gap-2 px-6">
          ...
          <div className="w-full overflow-x-auto">
            <Table>
              ...
              <Table.Body>
                {!loading_app && data_table_items?.length ? (
                  data_table_items.map((item: any, i: number) => (
                    <Table.Row key={i}>
                      <Table.Cell className="flex flex-row items-center justify-end gap-2">
                        <Tooltip content="Editar" align="top">
                          <a href={`/items/${item?.sku}/edit`}>
                            <LuPencil className="pencil_edit opacity-0 text-lg cursor-pointer" />
                          </a>
                        </Tooltip>
                      </Table.Cell>
                      <Table.Cell mainCol>
                        <a href={`/items/${item?.sku}`} className="text-[#F9004D] underline">
                          {item?.sku}
                        </a>
                      </Table.Cell>
                      <Table.Cell>{item?.name}</Table.Cell>
                      <Table.Cell>{item?.storePickup}</Table.Cell>
                    </Table.Row>
                  ))
                ) : (
                  <Table.Row>
