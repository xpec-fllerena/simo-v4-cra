...
import { get_data_table_stock } from "helpers/data_tables"
...
const ListScreen = () => {
  const { loading_app, data, search_osrm_action } = useSearchOSRM({ entity: "stocks" })
  ...
  const data_table_stocks = get_data_table_stock(current_data)

  return (
    <div className="w-full h-full flex flex-col items-center justify-start">
      <Breadcrumb data={[{ name: "Listado de stock", href: "/stock" }]} />
      <div className="w-full flex flex-col justify-center items-center py-4 px-6">
        <TitleScreen title="listado de stock" />
        {/* <p>StockListScreen</p> */}
      </div>
      <div className="w-full flex flex-col gap-2">
        <div className="w-full flex flex-col lg:flex-row justify-between items-end lg:items-center gap-4 px-6">
          <SearchBarFilter />
          <ButtonListNavigation label="Crear Stock" route="/stock/new" />
        </div>
        <div className="w-full flex flex-col gap-2 px-6">
          ...
          <div className="w-full overflow-x-auto">
            <Table>
              ...
              <Table.Body>
                {!loading_app && data_table_stocks?.length ? (
                  data_table_stocks.map((stock: any, i: number) => (
                    <Table.Row key={i}>
                      ...
                      <Table.Cell mainCol>
                        <a href={`/stock/${stock?.id}`} className="text-[#F9004D] underline">
                          {stock?.id}
                        </a>
                      </Table.Cell>
