...
import { get_data_table_locations } from "helpers/data_tables"
...
const ListScreen = () => {
  const { loading_app, data, search_osrm_action } = useSearchOSRM({ entity: "locations" })
  ...
  const data_table_locations = get_data_table_locations(current_data)

  return (
    <div className="w-full h-full flex flex-col items-center justify-start">
      <Breadcrumb data={[{ name: "Listado de ubicaciones", href: "/locations" }]} />
      <div className="w-full flex flex-col justify-center items-center py-4 px-6">
        <TitleScreen title="listado de ubicaciones" />
      ...
      <div className="w-full flex flex-col gap-2">
        <div className="w-full flex flex-col lg:flex-row justify-between items-end lg:items-center gap-4 px-6">
          <SearchBarFilter />
          <ButtonListNavigation label="Crear Ubicación" route="/locations/new" />
        </div>
          ...
          <div className="w-full overflow-x-auto">
            <Table>
              <Table.Head>
                ...
                <Table.HeadCell>criteria0</Table.HeadCell>
                <Table.HeadCell>criteria1</Table.HeadCell>
                <Table.HeadCell>criteria2</Table.HeadCell>
                <Table.HeadCell>criteria3</Table.HeadCell>
              </Table.Head>
              <Table.Body>
                {!loading_app && data_table_locations?.length ? (
                  data_table_locations.map((location: any, i: number) => (
                    <Table.Row key={i}>
                      ...
                      <Table.Cell mainCol>{location?.id}</Table.Cell>
                      <Table.Cell>{location?.criteria0}</Table.Cell>
                      <Table.Cell>{location?.criteria1}</Table.Cell>
                      <Table.Cell>{location?.criteria2}</Table.Cell>
                      <Table.Cell>{location?.criteria3}</Table.Cell>
                    </Table.Row>
                  ))
                ) : (
                  <Table.Row>
